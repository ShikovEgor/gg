        ...
        for _ in range(arch['nlayers_gnn']):
            if arch['conv_type'] == 'sage':
                self.conv_layers.append(GCNConv(arch['nd'], arch['nd'], 
                        aggr=arch['node_agg_type'], add_self_loops =False))
                ...
                self.conv_layers.append(GATConv(arch['nd'], arch['nd'], heads = 8, add_self_loops  =False,concat =False,
                        aggr=arch['node_agg_type'])) 
...
# weight_norm(
        ...
        self.lin_fin_0 = nn.Linear(1, 1)
        self.lin_fin_0.bias.data.fill_(b0)
        if w0 is not None:
            self.lin_fin_0.weight.data.fill_(-0.01)
            
        # for heterophily setting
        self.f1 = weight_norm(nn.Linear(nd, nd))
        self.f2 = weight_norm(nn.Linear(nd, nd))
        self.ncomp = 8
        self.dhead = nd // self.ncomp
        self.lin_fin_mix = nn.Linear(self.ncomp, 1)
        
        self.alpha = nn.Parameter(torch.ones(self.ncomp))       
        self.bias = nn.Parameter(torch.Tensor([0.]))       
        self.ww = nn.Parameter(torch.Tensor([0.001]))       
        self.coeff = nn.Parameter(torch.ones(4))     
        self.lin_h = weight_norm(nn.Linear(self.ncomp, 1))
    ...
    def forward(self,  v1, v2):
        ...
        elif self.fin_type == 'l2_sum':
            z =  ((v1-v2)**2).sum(dim=-1)
            z = self.lin_fin_0(torch.unsqueeze(z, -1)) #z.view(-1,1)
             
        elif self.fin_type == 'diff_het':
            v11, v12 = self.f1(v1), self.f1(v2)
            v21, v22 = self.f2(v1), self.f2(v2)
            
            v11 = F.normalize(v11, dim=-1)
            v12 = F.normalize(v12, dim=-1)
            v21 = F.normalize(v21, dim=-1)
            v22 = F.normalize(v22, dim=-1)
#             k = F.softmax(self.coeff)
#             z = k[0]*torch.abs(v11- v22) + k[1]*torch.abs(v12- v21)+k[2]*torch.abs(v11- v12) + k[3]*torch.abs(v21- v22)
            
            z = torch.abs(v11- v12) + torch.abs(v12- v21)+torch.abs(v11- v12) + torch.abs(v21- v22)
#             z = (v11- v12)**2
            z = z.view((-1,self.ncomp,self.dhead)).mean(dim=-1)
#             z = self.lin_fin_mix(z)
#             z = torch.matmul(torch.sigmoid(z), F.softmax(self.alpha))
#             z = z.mean(dim= -1)*self.ww  +self.bias
#             z = torch.logit(z, eps=1e-15).view(-1,1)+self.bias
#             print(z)


            z = self.lin_h(z)*self.ww  +self.bias
        return z.squeeze()            
...
class KernelAtt(nn.Module):
    def __init__(self, nd, w0 = None, b0 = 0):
        ...
        phi = GeneralizedRandomFeatures(n_pos, n_dims = 32)
        phi.new_feature_map(device = torch.device('cpu'))
